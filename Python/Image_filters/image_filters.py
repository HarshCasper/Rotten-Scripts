...
b_kernel = np.array(list(map(int, input(
    "Enter the kernel to be applied to the blue stream").split()))).reshape(3, 3)
r_kernel = np.array(list(map(int, input(
    "Enter the kernel to be applied to the red stream").split()))).reshape(3, 3)
g_kernel = np.array(list(map(int, input(
    "Enter the kernel to be applied to the green stream").split()))).reshape(3, 3)
...
def convolute(arr, height, width, kernel):
    ...
    convoluted_matrix = (
        np.array(convoluted_matrix).reshape(height-3, width-3))
...
for i in range(height):
    for j in range(width):
        combine[i, j] = [int(r_convol[i][j]), int(
            g_convol[i][j]), int(b_convol[i][j])]
