...
async def softban(ctx, member: discord.Member, *, reason=None):
    if ctx.author == member:
        ...
        return
    if member.guild_permissions.administrator or member.guild_permissions.kick_members or member.guild_permissions.kick_members:
        await ctx.send(
            f"That User is a Administrator/Moderator so i cant {ctx.command.name} that user", delete_after=7)
        return
    if ctx.guild.me.top_role.position < member.top_role.position:
        await ctx.send(
            f"My Role Is Not High Enough To {ctx.command.name} This User", delete_after=7)
        return
    if not reason:
        reason = "None"
...
async def kick(ctx, member: discord.Member, *, reason=None):
    ...
    if not reason:
        reason = "None"
    try:
        await member.send(f"You have been kicked from `{ctx.guild.name}` Reason: \n{reason}")
        ...
        embed.set_footer(
            text=f"Executor: {ctx.author} ID: {ctx.author.id}", icon_url=ctx.author.avatar_url)
        await ctx.send(embed=embed)
...
async def serverinfo(ctx):
    ...
    embed.add_field(name="Server Owner:",
                    value=ctx.message.guild.owner.mention)
    embed.add_field(name="Total Members:", value=ctx.guild.member_count)
    embed.add_field(name="Server Verification Level",
                    value=ctx.guild.verification_level)
    embed.add_field(name='Currently Online', value=str(online))
    embed.add_field(
        name='Statues', value=f"🟢 {statues[0]} 🌙 {statues[1]}\n🔴 {statues[2]} ⚪ {statues[3]}")
    embed.add_field(name="Server Created at",
                    value=ctx.guild.created_at.strftime("%a, %#d %B %Y, %I:%M %p UTC (Universal Time Coordinated)"))
...
async def userinfo(ctx, member: discord.Member = None):
    embed = discord.Embed(
        color=discord.Colour.blue(),
        timestamp=ctx.message.created_at, title="User Information")
    if not member:
        member = ctx.author
    roles = [role for role in member.roles]
    join_position = sorted(
        ctx.guild.members, key=lambda m: m.joined_at).index(member) + 1
    ...
    embed.add_field(name="All Roles:", value="".join(
...
async def on_command_error(ctx, error):
    if isinstance(error, commands.NoPrivateMessage):
        ...
        return await ctx.send(f"It looks like you are missing some permissions\n `{', '.join(error.missing_perms)}`")
