...
async def kick(ctx, member: discord.Member, *, reason=None):
    ...
    try:
        ...
        embed.set_footer(text=f"Executor: {ctx.author} ID: {ctx.author.id}", icon_url=ctx.author.avatar_url)
        await ctx.send(embed=embed)
@commands.bot_has_guild_permissions(ban_members=True)
@commands.has_permissions(ban_members=True)
@commands.guild_only()
...
async def ban(ctx, member: discord.Member, *, reason=None):
    ...
    try:
        ...
        embed.set_footer(text=f"{ctx.author} ID: {ctx.author.id}",icon_url=ctx.author.avatar_url)
        await ctx.send(embed=embed)
...
async def serverinfo(ctx):
    ...
    embed.add_field(name="Server Verification Level", value=ctx.guild.verification_level)
    embed.add_field(name='Currently Online', value=str(online))
    embed.add_field(name='Statues', value=f"🟢 {statues[0]} 🌙 {statues[1]}\n🔴 {statues[2]} ⚪ {statues[3]}")
