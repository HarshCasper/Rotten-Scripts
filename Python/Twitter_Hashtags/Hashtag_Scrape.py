...
file_name = input('Enter the name of file to store the tweet info (along with file path): ')
...
for i in sntweets.TwitterSearchScraper('#'+tag).get_items():
    tweets_list.append([i.id, i.user.username, i.date, i.content, i.replyCount, i.retweetCount, i.likeCount, i.quoteCount, i.url])
    count += 1
...
if count != 0:
    header_list = ['Id', 'Username', 'Date and Time', 'Tweet', 'Reply count', 'Retweet count', 'Like count', 'Quote count', 'Url']
