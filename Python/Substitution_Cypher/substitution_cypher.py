...
def encrypt(text, key):
    ...
    for i in range(0, len(text)):
        if text[i] == " ":
            ...
            enc_string = enc_string + \
                i_a[(a_i[text[i].upper()]+key) % 26].upper()
        else:
            enc_string = enc_string + \
                i_a[(a_i[text[i].upper()]+key) % 26].lower()
    return enc_string
...
def decrypt(enc_text, key):
    ...
    for i in range(0, len(enc_text)):
        if enc_text[i] == " ":
            ...
            dec_string = dec_string + \
                i_a[abs((a_i[enc_text[i].upper()]-key)) % 26].upper()
        else:
            dec_string = dec_string + \
                i_a[abs((a_i[enc_text[i].upper()]-key)) % 26].lower()
    return dec_string
...
def try_again():
    import sys
    retry = input(
        "Do you want to try again. Enter 1 for yes and anything for no")
    if(retry != '1'):
        sys.exit()
