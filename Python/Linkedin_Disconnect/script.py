...
def username_disconnected(browser):
    ...
    for user in Users:
        ...
        element=browser.find_element_by_css_selector('span[aria-label="More actions"]')
        element.click()
        if soup.find("div",{"data-control-name":"disconnect"})!=None:
            element=browser.find_element_by_css_selector('div[data-control-name="disconnect"]')
            element.click()
            print(f"Disconnected User {user}")

        if soup.find("div",{"data-control-name":"unfollow"})!=None:    
            element=browser.find_element_by_css_selector('div[data-control-name="unfollow"]')
            element.click()
...
def profilename_disconnected(browser):
    ...
    login(browser)
    linkedin_connection_url="https://www.linkedin.com/mynetwork/invite-connect/connections/"
    ...
    for user in Users:
        ...
        time.sleep(1) 
        removed_btn=browser.find_element_by_css_selector('.mn-connection-card__action-container button.artdeco-button--circle')

        removed_btn.click()
        time.sleep(2)
        element=browser.find_element_by_css_selector('div.artdeco-dropdown__content-inner')
        element.click()
        time.sleep(2)
        element=browser.find_elements_by_css_selector('div[role="alertdialog"] span.artdeco-button__text')[2]
        element.click()
...
def main():
    ...
    print("If You have Added Profile Name of User Enter choice 1 or if you have added Linkedin-username of User Enter Choice 2")
