...
def print_output(result, verbose, state, col):
    ...
    if verbose:
        ...
        for i in result['data']['repository']['pullRequests']['nodes']:
            ...
            lname = truncate(
                ", ".join(json_extract(i, 'name')), math.ceil(col/5))
            rprint(f"[bold {state_color[state]}]#{number[0]}[/bold {state_color[state]}]",
                   title.ljust(math.ceil(col/4)), f"( {lname} )".ljust(math.ceil(col/4)), dates)
    else:
        click.secho(state, fg=state_color[state], bold=True)
        print("\n")
        for i in result['data']['repository']['pullRequests']['nodes']:
