...
def main():
    intro()
    parser = argparse.ArgumentParser(description="Send Emails faster to masses in just one command")
    ...
    """
    parser_send = subparser.add_parser('send',help = "Send emails to masses using this command")
    ...
    parser_send.add_argument('-path',type= str,help = "Path to CSV file containing list of reciever's email ids")
    ...
    try:
        ...
        if (args.command == 'register'):
            ...
            elif(args.sub is not None and args.text is not None and args.path is not None):
